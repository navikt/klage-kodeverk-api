...
val hjemmelComparator =
    Comparator<String> { value1, value2 ->
...
private fun String.toComparisonList() =
    digitRegex
        .findAll(input = this)
        .mapNotNull { it.groupValues.first().takeIf { s -> s.isNotBlank() } }
        .map { it.lowercase() }
        .toList()

private val numberOrOrdinalOrStringComparator =
    Comparator<String> { value1, value2 ->
...
// The entry names are matched against the text being sorted (Ordinal.entries.find { it.name == value }),
// so they have to stay lowercase Norwegian words. Renaming them would silently break the sorting.
